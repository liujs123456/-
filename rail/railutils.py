...
def get_M_Minv():
    ...
    src = np.float32([[(400, 1080), (700, 200), (1700, 1080), (1100, 200)]])/2
    dst = np.float32([[(500, 1080), (0, 0), (1500, 1080), (1300, 0)]])/2
...
def draw_area(undist,binary_warped,Minv,left_fit, right_fit):
    ...
    # plt.imshow(undist)  # 使用 'gray' 颜色映射以显示黑白图像
    ...
    # 转换为灰度图
    gray_newwarp = cv2.cvtColor(newwarp, cv2.COLOR_BGR2GRAY)

    # thresh现在是一个单通道、仅包含0和1的二值化图像
    threshold_value = 100  # 根据实际情况调整合适的阈值
    thresh = cv2.threshold(gray_newwarp, threshold_value, 1, cv2.THRESH_BINARY)[1]
    
    # plt.imshow(thresh, cmap='gray')  # 使用 'gray' 颜色映射以显示黑白图像
    # plt.axis('off')  # 可选：关闭坐标轴显示，使图像更整洁
    # plt.show()
    print(thresh.shape)
    
    return result,newwarp,thresh
